...
# Ajouter les hostnames dans /etc/hosts pour que le host puisse résoudre les noms des CAs et des nœuds
echo "Configuration de /etc/hosts pour la résolution des noms..."
CA_HOSTS="tls-ca.finance.com root-ca.finance.com int-ca.finance.com orderer1.finance.com orderer2.finance.com orderer3.finance.com peer1-org1.finance.com peer2-org1.finance.com peer1-org2.finance.com peer2-org2.finance.com"
for h in $CA_HOSTS; do
  if ! grep -q "$h" /etc/hosts; then
    echo "127.0.0.1 $h" | sudo tee -a /etc/hosts > /dev/null
    echo "  Ajouté $h dans /etc/hosts"
  else
    echo "  $h déjà présent dans /etc/hosts"
  fi
done
echo "Résolution des noms configurée."
...
# Création du réseau Docker externe pour tous les services
echo "Création du réseau Docker..."
docker network rm fabric_network 2>/dev/null || true
docker network rm hyperledger-fabric-network_fabric_network 2>/dev/null || true
docker network create --driver bridge --subnet 172.18.0.0/16 --gateway 172.18.0.1 fabric_network 2>/dev/null || true
...
# Vérification que le réseau Docker fabric_network existe toujours
if ! docker network inspect fabric_network >/dev/null 2>&1; then
  echo "Recréation du réseau Docker fabric_network..."
  docker network create --driver bridge --subnet 172.18.0.0/16 --gateway 172.18.0.1 fabric_network
fi
echo "Réseau Docker vérifié."
...
# Créer docker-compose pour le chaincode avec réseau externe
...
for org in {1..2}; do
  PEER_PORT_VAR="PEER1_ORG${org}_PORT"
  PEER_PORT=${!PEER_PORT_VAR}
  docker exec fabric-cli env \
    FABRIC_LOGGING_SPEC=INFO \
    CORE_PEER_LOCALMSPID=Org${org}MSP \
...
# ===================================================================
# TESTS COMPLETS DU NOUVEAU CHAINCODE BANCAIRE
# ===================================================================
echo "=== TESTS COMPLETS DU CHAINCODE BANCAIRE ==="
sleep 10

# 1. InitLedger
echo "1. InitLedger..."
docker exec fabric-cli peer chaincode invoke -o orderer1.finance.com:${ORDERER1_PORT} --ordererTLSHostnameOverride orderer1.finance.com \
  --tls --cafile /etc/hyperledger/orderer/tls/tlscacerts/tls-ca-cert.pem \
  -C tokenchannel -n token -c '{"function":"InitLedger","Args":[]}' \
  --peerAddresses peer1-org1.finance.com:${PEER1_ORG1_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem \
  --peerAddresses peer1-org2.finance.com:${PEER1_ORG2_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem
if [ $? -ne 0 ]; then
  echo "Erreur : échec de InitLedger."
  exit 1
fi
sleep 10

# 2. CreateAccount
echo "2. CreateAccount..."
docker exec fabric-cli peer chaincode invoke -o orderer1.finance.com:${ORDERER1_PORT} --ordererTLSHostnameOverride orderer1.finance.com \
  --tls --cafile /etc/hyperledger/orderer/tls/tlscacerts/tls-ca-cert.pem \
  -C tokenchannel -n token -c '{"function":"CreateAccount","Args":["ACC007","BNK005","MAD","Investment","50000"]}' \
  --peerAddresses peer1-org1.finance.com:${PEER1_ORG1_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem \
  --peerAddresses peer1-org2.finance.com:${PEER1_ORG2_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem
if [ $? -ne 0 ]; then
  echo "Erreur : échec de CreateAccount."
  exit 1
fi
sleep 10

# 3. GetAllAccounts
echo "3. GetAllAccounts..."
docker exec fabric-cli peer chaincode query -C tokenchannel -n token -c '{"function":"GetAllAccounts","Args":[]}'

# 4. Mint
echo "4. Mint (to ACC001)..."
docker exec fabric-cli peer chaincode invoke -o orderer1.finance.com:${ORDERER1_PORT} --ordererTLSHostnameOverride orderer1.finance.com \
  --tls --cafile /etc/hyperledger/orderer/tls/tlscacerts/tls-ca-cert.pem \
  -C tokenchannel -n token -c '{"function":"Mint","Args":["ACC001","2500"]}' \
  --peerAddresses peer1-org1.finance.com:${PEER1_ORG1_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem \
  --peerAddresses peer1-org2.finance.com:${PEER1_ORG2_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem
if [ $? -ne 0 ]; then
  echo "Erreur : échec de Mint."
  exit 1
fi
sleep 10

# 5. Transfer
echo "5. Transfer (ACC001 -> ACC002)..."
docker exec fabric-cli peer chaincode invoke -o orderer1.finance.com:${ORDERER1_PORT} --ordererTLSHostnameOverride orderer1.finance.com \
  --tls --cafile /etc/hyperledger/orderer/tls/tlscacerts/tls-ca-cert.pem \
  -C tokenchannel -n token -c '{"function":"Transfer","Args":["ACC001","ACC002","800"]}' \
  --peerAddresses peer1-org1.finance.com:${PEER1_ORG1_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem \
  --peerAddresses peer1-org2.finance.com:${PEER1_ORG2_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem
if [ $? -ne 0 ]; then
  echo "Erreur : échec de Transfer."
  exit 1
fi
sleep 10

# 6. GetAllTransactions
echo "6. GetAllTransactions..."
docker exec fabric-cli peer chaincode query -C tokenchannel -n token -c '{"function":"GetAllTransactions","Args":[]}'

# 7. CreateUser
echo "7. CreateUser..."
docker exec fabric-cli peer chaincode invoke -o orderer1.finance.com:${ORDERER1_PORT} --ordererTLSHostnameOverride orderer1.finance.com \
  --tls --cafile /etc/hyperledger/orderer/tls/tlscacerts/tls-ca-cert.pem \
  -C tokenchannel -n token -c '{"function":"CreateUser","Args":["USR005","Ahmed El Amrani","ahmed@finance.ma","Auditor","true"]}' \
  --peerAddresses peer1-org1.finance.com:${PEER1_ORG1_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem \
  --peerAddresses peer1-org2.finance.com:${PEER1_ORG2_PORT} --tlsRootCertFiles /etc/hyperledger/peer/tls/tlscacerts/tls-ca-cert.pem
if [ $? -ne 0 ]; then
  echo "Erreur : échec de CreateUser."
  exit 1
fi
sleep 10

# 8. GetDashboardStats
echo "8. GetDashboardStats..."
docker exec fabric-cli peer chaincode query -C tokenchannel -n token -c '{"function":"GetDashboardStats","Args":[]}'
...
echo "======================================"
echo "Réseau Hyperledger Fabric déployé et testé avec succès !"
echo "======================================"
echo ""
echo "Vous pouvez interagir avec le réseau en utilisant le conteneur CLI :"
echo "docker exec -it fabric-cli /bin/sh"
echo ""
echo "Exemple : pour lister tous les comptes, exécutez dans le conteneur :"
echo "peer chaincode query -C tokenchannel -n token -c '{\"function\":\"GetAllAccounts\",\"Args\":[]}'"